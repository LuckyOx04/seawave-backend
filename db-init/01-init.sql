...
CREATE PROCEDURE IF NOT EXISTS sp_RegisterUser(IN p_username VARCHAR(50), IN p_email VARCHAR(255), IN p_password_hash VARCHAR(255))
BEGIN 
    INSERT INTO users (username, email, password_hash) VALUES (p_username, p_email, p_password_hash);
    SELECT LAST_INSERT_ID() AS UserId;
END //

CREATE PROCEDURE IF NOT EXISTS sp_GetUserById(IN p_user_id INT)
BEGIN 
    SELECT id AS Id, username AS Username, email AS Email, password_hash AS PasswordHash, is_confirmed AS IsConfirmed FROM users
    WHERE id = p_user_id;
END //

CREATE PROCEDURE IF NOT EXISTS sp_GetUserByLogin(IN p_identifier VARCHAR(255))
BEGIN
    SELECT id AS Id, username AS Username, email AS Email, password_hash AS PasswordHash, is_confirmed AS IsConfirmed FROM users
    WHERE username = p_identifier OR email = p_identifier;
END //

CREATE PROCEDURE IF NOT EXISTS sp_SetPasswordResetToken(IN p_email VARCHAR(255), IN p_token_hash VARCHAR(255), IN p_expiry_minutes INT)
BEGIN 
...
CREATE PROCEDURE IF NOT EXISTS sp_ResetPasswordWithToken(IN p_token_hash VARCHAR(255), IN p_new_password_hash VARCHAR(255))
BEGIN 
    DECLARE v_user_id INT;
...
CREATE PROCEDURE IF NOT EXISTS sp_ChangePassword(IN p_user_id INT, IN p_new_password_hash VARCHAR(255))
BEGIN 
    UPDATE users SET password_hash = p_new_password_hash WHERE id = p_user_id;
END //

CREATE PROCEDURE IF NOT EXISTS sp_SearchTracks(IN p_query VARCHAR(255))
BEGIN 
...
CREATE PROCEDURE IF NOT EXISTS sp_SearchPlaylists(IN p_query VARCHAR(255))
BEGIN 
...
CREATE PROCEDURE IF NOT EXISTS sp_GetPlaylistById(IN p_playlist_id INT)
BEGIN
    SELECT playlists.id AS Id, playlists.name AS Name, playlists.user_id AS CreatorId, users.username AS CreatorName
...
CREATE PROCEDURE IF NOT EXISTS sp_GetPlaylistByUserId(IN p_user_id INT)
BEGIN
    SELECT playlists.id AS Id, playlists.name AS Name, playlists.user_id AS CreatorId, users.username AS CreatorName,
    (SELECT COUNT(*) FROM playlists_tracks pt WHERE pt.playlist_id = playlists.id) AS TrackCount
    FROM playlists
...
CREATE PROCEDURE IF NOT EXISTS sp_GetPlaylistTracks(IN p_playlist_id INT)
BEGIN 
    SELECT id AS Id, title AS Title, artist AS Artist, file_name AS FileName, duration_seconds AS DurationSeconds FROM tracks
...
CREATE PROCEDURE IF NOT EXISTS sp_CreatePlaylist(IN p_name VARCHAR(100), IN p_user_id INT)
BEGIN 
    INSERT INTO playlists (name, user_id) VALUES (p_name, p_user_id);
    SELECT LAST_INSERT_ID() AS PlaylistId;
END //

CREATE PROCEDURE IF NOT EXISTS sp_DeletePlaylist(IN p_user_id INT, IN p_playlist_id INT)
BEGIN 
    DELETE FROM playlists WHERE id = p_playlist_id AND user_id = p_user_id;
    SELECT ROW_COUNT() AS Success;
END //

CREATE PROCEDURE IF NOT EXISTS sp_AddTrackToPlaylist(IN p_user_id INT, IN p_playlist_id INT, IN p_track_id INT)
BEGIN 
...
CREATE PROCEDURE IF NOT EXISTS sp_RemoveTrackFromPlaylist(IN p_user_id INT, IN p_playlist_id INT, IN p_track_id INT)
BEGIN 
...
CREATE PROCEDURE IF NOT EXISTS sp_RequestUpload(In p_user_id INT, IN p_title VARCHAR(255), IN p_artist VARCHAR(255), IN p_temp_file_name VARCHAR(500))
BEGIN 
    INSERT INTO upload_queue (user_id, title, artist, file_name)
    VALUES (p_user_id, p_title, p_artist, p_temp_file_name);
END //

CREATE PROCEDURE IF NOT EXISTS sp_CreateSession(IN p_session_token CHAR(36), IN p_user_id INT, IN p_expiry_days INT)
BEGIN 
    INSERT INTO user_sessions (session_token, user_id, expires_at)
    VALUES (p_session_token, p_user_id, DATE_ADD(NOW(), INTERVAL p_expiry_days DAY));
END //

CREATE PROCEDURE IF NOT EXISTS sp_ValidateSession(IN p_session_token CHAR(36))
BEGIN 
    SELECT user_id AS UserId FROM user_sessions
    WHERE session_token = p_session_token AND expires_at > NOW();
END //

CREATE PROCEDURE IF NOT EXISTS sp_DeleteSession(IN p_session_token CHAR(36))
BEGIN 
    DELETE FROM user_sessions WHERE session_token = p_session_token;
END //

CREATE PROCEDURE IF NOT EXISTS sp_SetEmailVerificationToken(IN p_user_id INT, IN p_token_hash CHAR(36))
BEGIN 
...
CREATE PROCEDURE IF NOT EXISTS sp_ConfirmEmail(IN p_token_hash CHAR(36))
BEGIN 
    DECLARE v_user_id INT;
...
CREATE PROCEDURE IF NOT EXISTS sp_GetUserProfileInfo(IN p_user_id INT)
BEGIN 
...
GRANT EXECUTE ON seawave.* TO 'seawave_db_api_user'@'localhost';
REVOKE ALL PRIVILEGES, GRANT OPTION FROM 'seawave_db_api_user'@'localhost';
GRANT EXECUTE ON seawave.* TO 'seawave_db_api_user'@'localhost';
